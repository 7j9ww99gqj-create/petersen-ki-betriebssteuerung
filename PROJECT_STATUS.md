...
- Aktuell in Arbeit am `2026-05-12`:
  ...
  - Dokumentauswahl im Büro ist jetzt strenger: bereits anders verknüpfte Dateien werden in Angebots-, Auftrags-, Rechnungs- und Eingangsrechnungsformularen nicht mehr angeboten; beim Umhängen werden konkurrierende Relationsfelder lokal und per Update bereinigt.
  - `getEinkaufWareneingaenge()` hat einen kleinen Lesefallback erhalten und fällt bei gemischten Live-Schemata von `created_at` auf `id` zurück.
  ...
  - Büro-UI: Formulare für Angebot, Auftrag, Rechnung und Eingangsrechnung können archivierte Dokumente verknüpfen; Listen/Karten zeigen den Dokumentnamen sichtbar an; beim Umhängen/Löschen wird die Relation in `buero_dokumente` zurücksynchronisiert.
  ...
  - Offene Punkte: beide Migrationen live anwenden, die strengere Dokumentauswahl mit Echtdaten prüfen und danach dieselbe Denke für globale Archivsicht und echte Objekt-Detailseiten nutzen.
  - Tests: `npm run build` erfolgreich; `npm run lint` scheitert weiterhin an einer bestehenden lokalen ESLint-Installation (`Cannot find module './eslint'`).
